 ...
 * ---------------------
 * Fetches real whale transactions from whale-alert.io API.
...
const WHALE_ALERT_KEY =
  process.env.WHALE_ALERT_API_KEY ?? 'MQ9UMIfhnbX6uQpgPRPxX29wb7jvZZYX';

const MIN_VALUE_USD = 500_000; // $500k minimum
const LIMIT = 20;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: `Whale Alert API error ${res.status}: ${text}` },
        { status: res.status },
      );
    }
